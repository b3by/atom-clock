...
export default class AtomClockView {
  ...
  initialize() {
    ...
    this.subscriptions.add(atom.config.onDidChange('atom-clock.monospacedFont', () => {
      this.setConfigValues()
  ...
  setConfigValues() {
    ...
    this.monospacedFont = atom.config.get('atom-clock.monospacedFont')
  }
  ...
  adjustElementSize() {
    contentWidth = this.element.firstChild.getBoundingClientRect().width + 5
  ...
  setFont(isMonospaced) {
    this.element.style.fontFamily = isMonospaced ? 'monospace' : 'inherit'
  }
